...
| # | Що | Де | Блокує |
|---|-----|-----|--------|
| ~~B1~~ | ~~Double opt-in~~ ✅ **закрито** | — | — |
| B2 | Вписати **реальну поштову адресу** | Buttondown → Settings | лише розсилку (CAN-SPAM, штраф за кожен лист) |

**Про B1 (double opt-in).** Тумблера не існує: [Buttondown вимагає double opt-in для всіх розсилок](https://docs.buttondown.com/double-opt-in) за замовчуванням. Вимкнути можна лише через звернення в підтримку з обґрунтуванням, і платформа прямо відмовляє, якщо збір іде через звичайну форму без зовнішньої верифікації. Отже обіцянка на сайті («we'll first send you a confirmation email — you're only subscribed once you click it») виконується примусово.

Емпірична перевірка, якщо схочете переконатись: підпишіть тестову адресу і подивіться на неї в **Subscribers** — статус має бути **`unactivated`**, і лише після кліку в листі він стане **`regular`**. До кліку людина листів не отримує.
...
2. ~~Double opt-in~~ — забезпечується платформою, дій не потрібно.
